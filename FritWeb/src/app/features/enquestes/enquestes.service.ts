...
export interface ResultadoPregunta { encuestaPreguntaId: number; texto: string; tipo: PreguntaTipo; respuestas: number; media?: number; opciones: { encuestaOpcionId: number; texto: string; votos: number; porcentaje: number }[]; textos: string[]; }
export interface EncuestaDetalle { resumen: EncuestaResumen; permiteEditarRespuesta: boolean; visibilidadResultados: VisibilidadResultados; preguntas: EncuestaPregunta[]; miRespuesta?: RespuestaValor[]; resultados?: ResultadoPregunta[]; pendientes?: string[]; destinatarioIds?: number[]; participants?: string[]; }
