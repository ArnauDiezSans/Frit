...
export class EconomiaPageComponent {
  ...
  undoAssignments(row: EconomiaMoviment): void { if (!confirm('Vols eliminar totes les assignacions d’aquest moviment?')) return; this.service.undoAssignments(row.id).subscribe({ next: () => this.load(), error: () => this.message.set("No s'han pogut desfer les assignacions.") }); }
